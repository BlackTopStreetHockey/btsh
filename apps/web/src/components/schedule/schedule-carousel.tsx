...
export default function ScheduleCarousel() {
  ...
  const getGamesByTime = (games: Game[]) => {
    return games.reduce((acc: any, game: Game) => {
      ...
      return acc; 
    }, []).sort((a: GameTimeSlot, b: GameTimeSlot) => timeToHours(a.start) - timeToHours(b.start));
  }

  return (
    <div>
      <div ref={ref} className='flex flex-row gap-0 overflow-x-hidden'>
        {data?.results.map((gameDay: GameDay) => {
          const gamesByTime = getGamesByTime(gameDay.games);
          console.log(gamesByTime)
          return (
            <div key={gameDay.id} className='flex flex-row border shadow'>
              ...
              <div className='flex flex-col items-center'>
                <div className='flex-grow text-xs text-gray-400 text-center [writing-mode:vertical-lr] rotate-180'>EAST</div>
                <div className='flex-grow text-xs text-gray-400 text-center [writing-mode:vertical-lr] rotate-180'>WEST</div>
              </div>
                ...
                <div key={timeSlot.start} className='w-8'>
                  ...
                  <div className='flex flex-col gap-1'>
                    {[timeSlot.east, timeSlot.west].map((game: Game, i) => (
                      <div key={game.id} className={`w-8 pb-1 ${i === 0 ? 'border-b' : ''}`}>
                        <Link href={`/games/${game.id}`} className='flex flex-col justify-center items-center rounded hover:shadow-inner-lg cursor-pointer'>
                          <img className='rounded' src={game.away_team.logo} alt={game.away_team.short_name} width={24}/>
                          <img className='rounded' src={game.home_team.logo} alt={game.home_team.short_name} width={24}/>
                        </Link>
                      </div>
                    ))}
                  </div>
