    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="space-y-2">
        ...
        >
          ...
          <SelectContent>
            {teams && teams?.results.map((team: Team) => (
              <SelectItem key={team.id} value={team.short_name}>
                {team.name}
              </SelectItem>
            ))}
            {!teams && (
              <SelectItem value="NA">No teams found</SelectItem>
            )}
          </SelectContent>
        </Select>
        {teamsLoading && (
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center space-x-2">
            <Checkbox
              ...
              onCheckedChange={(checked) => {
                if (checked) {
                  updateFormData({
                    helpingInterests: [...formData.helpingInterests, "reffing"],
                  });
                } else {
                  updateFormData({
          ...
          <div className="flex items-center space-x-2">
            <Checkbox
              id="opening-closing"
              checked={formData.helpingInterests.includes("opening-closing")}
              onCheckedChange={(checked) => {
                if (checked) {
                  updateFormData({
                    helpingInterests: [
          ...
          <div className="flex items-center space-x-2">
            <Checkbox
              ...
              onCheckedChange={(checked) => {
                if (checked) {
                  updateFormData({
                    helpingInterests: [...formData.helpingInterests, "other"],
                  });
                } else {
                  updateFormData({
                    helpingInterests: formData.helpingInterests.filter(
          ...
          <div className="flex items-center space-x-2">
            <Checkbox
              ...
              onCheckedChange={(checked) => {
                if (checked) {
                  updateFormData({
                    helpingInterests: [...formData.helpingInterests, "next-year"],
                  });
                } else {
                  updateFormData({
                    helpingInterests: formData.helpingInterests.filter(
